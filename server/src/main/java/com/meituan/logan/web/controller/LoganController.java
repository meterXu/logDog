...
public class LoganController {
     ...
     * 获取全部日志类型列表
     */
    @GetMapping("/meta/logtypes")
    @ResponseBody
    public LoganResponse<List<Tuple<String, String>>> logTypes() {
     ...
     * 通过一个详情ID获取该条日志的详情信息
     */
    @GetMapping("/task/{detailId}/detail")
    @ResponseBody
    ...
    private String getFileName(int taskId) {
        if (taskId <= 0) {
            return null;
        }
        LoganTaskModel model = taskService.getByTaskId(taskId);
     ...
     */
    @GetMapping("/downing")
    public ResponseEntity<byte[]> downloadFile(@PathVariable("taskId") int taskId) throws IOException {
        String name = getFileName(taskId);
