...
public class WebLogParser {
    ...
    private static WebLogDetailDTO parseOneLogItem(String logItem, long taskId) {
        ...
        String environment = jsonObject.getString(WebLogFieldEnum.ENVIRONMENT.key);
        String webSource = jsonObject.getString(WebLogFieldEnum.WEB_SOURCE.key);
        String customInfo = jsonObject.getString(WebLogFieldEnum.CUSTOM_INFO.key);
        ...
        if (LocalStringUtils.isAllNotEmpty(logType, content, logTime)) {
            ...
            detailDTO.setLogType(logType);
            detailDTO.setEnvironment(environment);
            detailDTO.setWebSource(webSource);
            detailDTO.setCustomInfo(customInfo);
            detailDTO.setLogTime(NumberUtils.toLong(logTime));
            Timestamp ts=new Timestamp(detailDTO.getLogTime());
            Date date=new Date(ts.getTime());
            Calendar cal = Calendar.getInstance();
            cal.setTime(date);
            detailDTO.setRecordYear(cal.get(Calendar.YEAR));
            detailDTO.setRecordMonth(cal.get(Calendar.MONTH) + 1);
            detailDTO.setRecordDay(cal.get(Calendar.DAY_OF_MONTH));
