...
public class WebLogController {
    ...
    public LoganResponse<List<WebLogTaskDTO>> latestReport() {
        List<WebLogTaskDTO> taskDTOS = webTaskService.latest(20);
    ...
    @GetMapping("/query")
    @ResponseBody
    public LoganResponse<TableModel> query(int pageSize, int pageNum,String deviceName,String deviceId) {
        List<TaskIndexDTO> tasks = webTaskService.query(pageSize,pageNum,deviceName,deviceId);
        if (CollectionUtils.isNotEmpty(tasks)) {
            long total = webTaskService.total(deviceName,deviceId);
            TableModel  res = new TableModel();
            res.setTotal(total);
            res.setRows(tasks);
            return LoganResponse.success(res);
        }
        return LoganResponse.badParam("没有数据");
    ...
    public LoganResponse<List<List<WebLogIndex>>> detailIndex(String tasks, String logTypes,
        ...
        List<String> logTypeList = TypeSafeUtil.parseStrList(logTypes, REGEX);
     ...
     */
    @GetMapping("/download")
    public ResponseEntity<byte[]> downLoadLog(String name,String tasks){
        List<Long> taskIds = TypeSafeUtil.parseLongList(tasks, REGEX);
        taskIds = TypeSafeUtil.ignore(taskIds, 0L);
     ...
     * 根据页面检索条件匹配需要查看的日志
     */
    private void match(List<WebLogDetailDTO> cachedList, long taskId, List<String> logTypes,
    ...
    @ResponseBody
    public LoganResponse<List<TaskByDateDTO>> queryTaskByDate(String deviceId) {
        List<TaskByDateDTO> res = webTaskService.queryTaskByDate(deviceId);
    ...
    @GetMapping("/dateLogIndex")
    @ResponseBody
    public LoganResponse<List<WebLogIndex>> dateLogIndex(String deviceId, long logDate) {
        List<WebLogIndex> res = webLogDetailService.dateLogIndex(deviceId,logDate);
        if(CollectionUtils.isNotEmpty(res)){
            return LoganResponse.success(res);
        }
        return LoganResponse.badParam("没有数据");
    ...
    @GetMapping("/dateLogDetails")
    @ResponseBody
    public LoganResponse<List<WebLogDetailDTO>> dateLogDetails(String deviceId, long logDate) {
        List<WebLogDetailDTO> res = webLogDetailService.dateLogDetails(deviceId,logDate);
        if(CollectionUtils.isNotEmpty(res)){
            return LoganResponse.success(res);
        }
        return LoganResponse.badParam("没有数据");
